...
PoseAILiveLinkServer::PoseAILiveLinkServer(FPoseAIHandshake myHandshake, PoseAILiveLinkSingleSource* mySource,
                                           bool isIPv6, int32 portNum) :
	source_(mySource), handshake(myHandshake), port(portNum)
{
	...
	if (protocolType == FNetworkProtocolTypes::IPv6)
	{
		UE_LOG(LogTemp, Display,
		       TEXT("PoseAI LiveLink: Created Server on IPv6 link-Local address (begins with fe80:) and Port:%d"),
		       port);
	}
	else if (GetIP(myIP))
	{
	...
	else
	{
		UE_LOG(LogTemp, Display,
		       TEXT(
			       "PoseAI LiveLink: Created Server but can't determine your IP address.  You may not have a valid network adapter."
		       ));
	}

	UE_LOG(LogTemp, Warning, TEXT("now fucking server reated!!!"));
}
...
{
	bool canBind = false;
	TSharedRef<FInternetAddr> localIp = ISocketSubsystem::Get(PLATFORM_SOCKETSUBSYSTEM)->GetLocalHostAddr(
		*GLog, canBind);
...
void PoseAILiveLinkServer::ReceiveUDPDelegate(const FString& recvMessage, const FPoseAIEndpoint& endpointRecv)
{
	...
	// let's fucking test a single json ???
	...
	const FString InputString =
		"{\"version\":\"1.2.5\",\"userName\":\"iphone2Python\",\"sessionUUID\":\"8EF5BA6A-A73E-419A-9BDC-7E18759C180C\",\"deciveName\":\"iPhone13,4\"}";
	...
	if (!bSuccessful)
	{
		auto e = Reader2->GetErrorMessage();
		UE_LOG(LogTemp, Warning, TEXT("decode json error: %s "), *e);
	}
	check(bSuccessful);
	...
	{
		auto e2 = Reader->GetErrorMessage();
		UE_LOG(LogTemp, Warning, TEXT("json解码错误啊兄弟！！ %s , error: %s "), *recvMessage, *e2);
		...
		FLiveLinkLog::WarningOnce(NAME_JsonError, failKey,
		                          TEXT("PoseAI: failed to deserialize json object from %s, %s"),
		                          *endpointRecv.ToString(), *Reader->GetErrorMessage());
		return;
	}
	...
	if (HasValidConnection() && !sameAsCurrent)
	{
		...
		{
			//reject
			UE_LOG(LogTemp, Display, TEXT("PoseAI: Ignoring contact from %s as already engaged."),
			       *endpointRecv.ToString());
			//consider sending rejected connection a warning message
		}
	}
	else if (!HasValidConnection() && !sameAsCurrent)
	{
		// new connection
	...
	else
	{
		...
		{
			//is likely a repeat hello message
			SendHandshake();
		}
	}
}
...
void PoseAILiveLinkServer::InitiateConnection(TSharedPtr<FJsonObject> jsonObject, const FPoseAIEndpoint& endpointRecv)
{
	...
	if (!(jsonObject->TryGetStringField(fieldVersion, version)))
	{
		...
		FLiveLinkSubjectKey failKey = FLiveLinkSubjectKey(GUID_Error, FName(endpointRecv.ToString()));
		FLiveLinkLog::WarningOnce(NAME_NoAppVersion, failKey,
		                          TEXT("PoseAI: Incoming connection does not have a hello handshake"));
		return;
	}
	else if (!CheckAppVersion(version))
	{
		...
		FLiveLinkSubjectKey failKey = FLiveLinkSubjectKey(GUID_Error, FName(endpointRecv.ToString()));
		FLiveLinkLog::WarningOnce(NAME_AppVersionFail, failKey,
		                          TEXT("PoseAI: Please update the mobile app to at least version %s"),
		                          *requiredMinVersion);
	...
	lastConnection = FDateTime::Now();
	UE_LOG(LogTemp, Display, TEXT("PoseAI: received new contact from %s on port %d"), *(connectionName.ToString()),
	       endpointRecv.Port);
}
...
void PoseAILiveLinkServer::SendHandshake() const
{
	...
	else
	{
		...
		FLiveLinkLog::WarningOnce(NAME_HandshakeFail, failKey, TEXT("PoseAI: Unable to send the handshake to %s"),
		                          *(endpoint.ToString()));
	}
}
...
FName PoseAILiveLinkServer::ExtractConnectionName(TSharedPtr<FJsonObject> jsonObject,
                                                  const FPoseAIEndpoint& endpointRecv) const
{
...
{
	UE_LOG(LogTemp, Display, TEXT("PoseAILiveLink: App version %s vs required version %s."), *version,
	       *requiredMinVersion);
